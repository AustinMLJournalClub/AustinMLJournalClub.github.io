---
title: "Reading List"
about:
  template: solana
---

::: {.callout-note}
The Austin ML Journal Club concluded in August 2026. This list is kept as a record and is no longer accepting suggestions. See the [closing announcement](posts/202608-closing/index.qmd).
:::

This page collects papers, books, articles, videos, and other resources that community members suggested for future discussions. The club has concluded, so this list is now a frozen record of what was proposed but never discussed.

## Suggestions at the Time of Closing

Papers the club discussed are listed on the [Archives](archives.qmd) page.

- [Benchmarking Empirical Privacy Protection for Adaptations of Large Language Models](https://openreview.net/pdf?id=jY7fAo9rfK) (ICLR 2026)
- [In-the-Flow Agentic System Optimization for Effective Planning and Tool Use (AgentFlow)](https://openreview.net/pdf?id=Mf5AleTUVK) (ICLR 2026)
- [Position: When AI Decides Who Gets an Organ — Multi-Agentic AI Systems in Transplant Medicine Risk Amplifying Disparities](https://icml.cc/virtual/2026/poster/67102)
- [Position: AI Capabilities Are Not Increasing Exponentially](https://icml.cc/virtual/2026/poster/67206) ([preprint](https://arxiv.org/abs/2602.04836))
- [Position: Beyond Reasoning Zombies — AI Reasoning Requires Process Validity](https://icml.cc/virtual/2026/poster/67122) ([preprint](https://philarchive.org/rec/LAWPBR-3))
- [Position: Benchmarks Do Not Measure Deployment Readiness in Clinical AI](https://icml.cc/virtual/2026/poster/67107)
- [SCP: Accelerating Discovery with a Global Web of Autonomous Scientific Agents](https://arxiv.org/abs/2512.24189)
- [The Markovian Thinker: Architecture-Agnostic Linear Scaling of Reasoning](https://arxiv.org/abs/2510.06557)
- [Language models cannot reliably distinguish belief from knowledge and fact](https://www.nature.com/articles/s42256-025-01113-8)
- [Enhancing Retrieval-Augmented Generation: A Study of Best Practices](https://arxiv.org/abs/2501.07391)
- [Retrieval Augmented Generation (RAG) and Beyond: A Comprehensive Survey on How to Make your LLMs use External Data More Wisely](https://arxiv.org/abs/2409.14924)
- [Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG](https://arxiv.org/abs/2501.09136)
- [Energy-Based Transformers are Scalable Learners and Thinkers](https://arxiv.org/abs/2507.02092)
- [Angles Don't Lie: Unlocking Training-Efficient RL Through the Model's Own Signals](https://arxiv.org/abs/2506.02281)
- [Large Language Model Hacking: Quantifying the Hidden Risks of Using LLMs for Text Annotation](https://arxiv.org/abs/2509.08825)
- [Text-to-LoRA: Instant Transformer Adaption](https://arxiv.org/abs/2506.06105)
- [Poisoning Attacks on LLMs Require a Near-constant Number of Poison Sample](https://arxiv.org/abs/2510.07192)
- [Petri: An open-source auditing tool to accelerate AI safety research](https://alignment.anthropic.com/2025/petri/)
- [Holistic Agent Leaderboard: The Missing Infrastructure for AI Agent Evaluation](https://arxiv.org/abs/2510.11977)
- [Alignment Faking in Large Language Models](https://arxiv.org/abs/2412.14093)
- [MolmoAct: Action Reasoning Models that can Reason in Space](https://arxiv.org/abs/2508.07917)

## Previously Suggested Papers

The following were suggested by community members during our earlier meeting phases:

- [Solving olympiad geometry without human demonstrations](https://www.nature.com/articles/s41586-023-06747-5?_hsmi=291326409)
- [Sleeper Agents: Training Deceptive LLMs that Persist Through Safety Training](https://arxiv.org/abs/2401.05566)
- [TOFU: A Task of Fictitious Unlearning for LLMs](https://arxiv.org/abs/2401.06121)
- [Quantifying the impact of uninformative features on the performance of supervised classification and dimensionality reduction algorithms](https://pubs.aip.org/aip/aml/article/1/4/046118/2928862/Quantifying-the-impact-of-uninformative-features)
- [Fine-tuning Aligned Language Models Compromises Safety, Even When Users Do Not Intend To!](https://arxiv.org/abs/2310.03693)
- [A Mulching Proposal](https://arxiv.org/abs/1908.06166)
- [Evaluating and Mitigating Discrimination in Language Model Decisions](https://www.anthropic.com/index/evaluating-and-mitigating-discrimination-in-language-model-decisions)
- [Dive into Deep Learning: Coding Session #4 Attention Mechanism I (MLT Artificial Intelligence)](https://www.youtube.com/watch?v=baEQGk-CTwk&ab_channel=MLTArtificialIntelligence)
- [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401)
- [MiniLLM: Large Language Models on Consumer GPUs](https://github.com/kuleshov/minillm)
- [The TinyLlama project](https://github.com/jzhang38/TinyLlama)
- [On the Opportunities and Risks of Foundation Models](https://arxiv.org/abs/2108.07258)
- [Challenges in Deploying Machine Learning: a Survey of Case Studies](https://arxiv.org/abs/2011.09926)
- [Machine Learning and the Future of Bayesian Computation](https://arxiv.org/abs/2304.11251)